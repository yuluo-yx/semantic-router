...
class ClientRequestTest(SemanticRouterTestBase):
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    print(f"Request attempt {attempt + 1} failed, retrying in {RETRY_DELAY} seconds...")
    ...
    def test_basic_request_format(self):
        ...
        if response.status_code >= 400:
            passed = False
            failure_reason.append(f"Request was rejected with status code {response.status_code}")
        ...
        self.print_response_info(
            response,
            {
                ...
                "Response Model": response_json.get("model", "Not specified") if passed else "N/A"
            }
        )

        self.print_test_result(
            passed=passed,
            message=", ".join(failure_reason) if failure_reason else "All checks passed"
        )
    ...
    def test_malformed_request(self):
        ...
        try:
            response = self._make_request(payload, timeout=10)  # Reduced timeout for error cases
            if not response:
                response = requests.post(
                    f"{ENVOY_URL}{OPENAI_ENDPOINT}",
                    headers={"Content-Type": "application/json"},
            ...
            self.print_test_result(
                passed=passed,
                message=f"Got expected 4xx status code: {response.status_code}" if passed else
                       f"Expected 4xx status code, got {response.status_code}"
            )

            self.assertTrue(passed, f"Expected 4xx status code, got {response.status_code}")
    ...
    def test_domain_specific_queries(self):
        ...
        for test_case in test_cases:
            with self.subTest(test_case["name"]):
                ...
                response = self._make_request(payload, timeout=60)  # Increased timeout for model responses
                if not response:
                    self.fail(f"Failed to get response for {test_case['name']} after retries")
                ...
                self.print_response_info(
                    response,
                    {
                        "Query Type": test_case["expected_type"],
                ...
                self.print_test_result(
                    passed=passed,
                    message=f"Successfully routed to model: {model}" if passed else
                           f"Request failed with status {response.status_code} or invalid response"
                )
