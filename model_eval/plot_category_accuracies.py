...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot MMLU-Pro accuracies by category"
    )
    parser.add_argument(
        "--results-dir",
        type=str,
...
def generate_sample_data():
    ...
    models = ["gpt-4:cot", "gpt-4:direct", "claude-3:cot", "claude-3:direct", "llama-3:cot"]
...
def create_bar_plot(category_accuracies, output_file):
    ...
    for i, model in enumerate(all_models):
        accuracies = [category_accuracies[category].get(model, 0) for category in categories]
...
def main():
    ...
    if (not category_accuracies or all(len(models) == 0 for models in category_accuracies.values())) and args.sample_data:
