...
def evaluate_category_classifier(model, tokenizer, texts_list, true_label_indices_list, idx_to_label_map, device):
    ...
    for text, true_label_idx in zip(texts_list, true_label_indices_list):
        predicted_category, confidence = predict_category(model, tokenizer, text, idx_to_label_map, device)
...
def main(model_name="minilm", num_epochs=5, batch_size=16):
    ...
    effective_batch_size = min(batch_size, 8)  # Smaller batches for better regularization
    ...
    logger.info(f"Starting MMLU-Pro category classification fine-tuning with {model_name}...")
    ...
    val_accuracy, val_report, val_conf_matrix, val_predictions = evaluate_category_classifier(
    ...
    test_accuracy, test_report, test_conf_matrix, test_predictions = evaluate_category_classifier(
...
def demo_inference(model_name="minilm"):
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        predicted_category, confidence = predict_category(model, tokenizer, text, idx_to_label, device)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MMLU-Pro Category Classification Fine-tuning")
